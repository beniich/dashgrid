...
const AIBlockRenderer = ({ block, onUpdate }: BlockRendererProps) => {
    ...
    const [output, setOutput] = useState((block.content as any)?.output || '');
    ...
    const handleGenerate = async () => {
        ...
        setTimeout(() => {
            ...
            onUpdate?.(block.id, {
                content: { ...(block.content as any), prompt, output: mockResponse }
            });
        ...
        case 'heading_1':
            return (
                <div className={containerClass}>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">{String(block.content)}</h1>
                </div>
            );
        case 'heading_2':
            return (
                <div className={containerClass}>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-4 mb-2">{String(block.content)}</h2>
                </div>
            );
        case 'heading_3':
            return (
                <div className={containerClass}>
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mt-2 mb-1">{String(block.content)}</h3>
                </div>
            );
        case 'text':
            ...
            return (
                <div className={containerClass}>
                    <p className="text-base text-gray-700 dark:text-gray-300 min-h-[1.5em]">{String(block.content)}</p>
                </div>
            );
            ...
            return (
                <div className={cn(containerClass, "flex gap-2 items-start")}>
                    ...
                    {!readOnly && onUpdate ? (
                        ...
                        <p className="text-base text-gray-700 dark:text-gray-300">{String(block.content)}</p>
                    )}
                </div>
            );
        case 'to_do':
